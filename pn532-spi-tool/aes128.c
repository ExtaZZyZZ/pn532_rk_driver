#include "aes128.h"
#include <string.h>

/* AES-128 ECB — compact implementation, encrypt + decrypt */

static const uint8_t sbox[256] = {
    ,
};

static const uint8_t rsbox[256] = {
    ,
};

static const uint8_t rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36,
};

static uint8_t xtime(uint8_t x) { return (x << 1) ^ ((x >> 7) * 0x1b); }
static uint8_t mul(uint8_t x, uint8_t y)
{
    return ((y & 1) * x) ^
           ((y >> 1 & 1) * xtime(x)) ^
           ((y >> 2 & 1) * xtime(xtime(x))) ^
           ((y >> 3 & 1) * xtime(xtime(xtime(x)))) ^
           ((y >> 4 & 1) * xtime(xtime(xtime(xtime(x)))));
}

static void key_expansion(const uint8_t *key, uint8_t w[11][4][4])
{
    memcpy(w[0], key, 16);
    for (int r = 1; r <= 10; r++) {
        uint8_t tmp[4];
        memcpy(tmp, w[r-1][3], 4);
        /* RotWord + SubWord + Rcon */
        uint8_t t = tmp[0];
        tmp[0] = sbox[tmp[1]] ^ rcon[r];
        tmp[1] = sbox[tmp[2]];
        tmp[2] = sbox[tmp[3]];
        tmp[3] = sbox[t];
        for (int c = 0; c < 4; c++) {
            for (int i = 0; i < 4; i++)
                w[r][c][i] = w[r-1][c][i] ^ tmp[i];
            memcpy(tmp, w[r][c], 4);
        }
    }
}

static void add_round_key(uint8_t s[4][4], uint8_t w[4][4])
{
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            s[r][c] ^= w[c][r];
}

static void sub_bytes(uint8_t s[4][4])
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            s[i][j] = sbox[s[i][j]];
}

static void inv_sub_bytes(uint8_t s[4][4])
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            s[i][j] = rsbox[s[i][j]];
}

static void shift_rows(uint8_t s[4][4])
{
    uint8_t t;
    t=s[1][0]; s[1][0]=s[1][1]; s[1][1]=s[1][2]; s[1][2]=s[1][3]; s[1][3]=t;
    t=s[2][0]; s[2][0]=s[2][2]; s[2][2]=t; t=s[2][1]; s[2][1]=s[2][3]; s[2][3]=t;
    t=s[3][3]; s[3][3]=s[3][2]; s[3][2]=s[3][1]; s[3][1]=s[3][0]; s[3][0]=t;
}

static void inv_shift_rows(uint8_t s[4][4])
{
    uint8_t t;
    t=s[1][3]; s[1][3]=s[1][2]; s[1][2]=s[1][1]; s[1][1]=s[1][0]; s[1][0]=t;
    t=s[2][0]; s[2][0]=s[2][2]; s[2][2]=t; t=s[2][1]; s[2][1]=s[2][3]; s[2][3]=t;
    t=s[3][0]; s[3][0]=s[3][1]; s[3][1]=s[3][2]; s[3][2]=s[3][3]; s[3][3]=t;
}

static void mix_columns(uint8_t s[4][4])
{
    for (int c = 0; c < 4; c++) {
        uint8_t a=s[0][c],b=s[1][c],cc=s[2][c],d=s[3][c];
        s[0][c]=mul(a,2)^mul(b,3)^cc^d;
        s[1][c]=a^mul(b,2)^mul(cc,3)^d;
        s[2][c]=a^b^mul(cc,2)^mul(d,3);
        s[3][c]=mul(a,3)^b^cc^mul(d,2);
    }
}

static void inv_mix_columns(uint8_t s[4][4])
{
    for (int c = 0; c < 4; c++) {
        uint8_t a=s[0][c],b=s[1][c],cc=s[2][c],d=s[3][c];
        s[0][c]=mul(a,0x0e)^mul(b,0x0b)^mul(cc,0x0d)^mul(d,0x09);
        s[1][c]=mul(a,0x09)^mul(b,0x0e)^mul(cc,0x0b)^mul(d,0x0d);
        s[2][c]=mul(a,0x0d)^mul(b,0x09)^mul(cc,0x0e)^mul(d,0x0b);
        s[3][c]=mul(a,0x0b)^mul(b,0x0d)^mul(cc,0x09)^mul(d,0x0e);
    }
}

static void bytes_to_state(const uint8_t *in, uint8_t s[4][4])
{
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            s[r][c] = in[c*4+r];
}

static void state_to_bytes(uint8_t s[4][4], uint8_t *out)
{
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            out[c*4+r] = s[r][c];
}

void aes128_encrypt(const uint8_t key[16], uint8_t data[16])
{
    uint8_t w[11][4][4];
    uint8_t s[4][4];
    key_expansion(key, w);
    bytes_to_state(data, s);
    add_round_key(s, w[0]);
    for (int r = 1; r <= 9; r++) {
        sub_bytes(s);
        shift_rows(s);
        mix_columns(s);
        add_round_key(s, w[r]);
    }
    sub_bytes(s);
    shift_rows(s);
    add_round_key(s, w[10]);
    state_to_bytes(s, data);
}

void aes128_decrypt(const uint8_t key[16], uint8_t data[16])
{
    uint8_t w[11][4][4];
    uint8_t s[4][4];
    key_expansion(key, w);
    bytes_to_state(data, s);
    add_round_key(s, w[10]);
    for (int r = 9; r >= 1; r--) {
        inv_shift_rows(s);
        inv_sub_bytes(s);
        add_round_key(s, w[r]);
        inv_mix_columns(s);
    }
    inv_shift_rows(s);
    inv_sub_bytes(s);
    add_round_key(s, w[0]);
    state_to_bytes(s, data);
}
